...
export default function UserInformation() {
    ...
    const is_log_in = () => {
        ...
        if (localStorage.getItem('fullname')) {
            navigate('/fuel_quote')
            alert('Infromation has already been given.')
        }
    }
    useEffect(() => {
        is_log_in();
    ...
    const set_user_info = async () => {
        ...
        let response = await fetch('http://localhost:5000/client/personal_information', {
            method: 'POST',
            headers: {
        ...
        <div>
            ...
            <div className='container d-flex justify-content-center' style={{ marginTop: '70px' }}>
                <div style={{ width: '80%', height: '450px', borderRadius: '20px', boxShadow: '0.5px 5px 10px #20202394' }}>
                    <h2 style={{ textAlign: 'center', marginTop: '10px' }}>Personal Information</h2>
